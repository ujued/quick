...
import net.apisp.quick.log.def.ConsoleLog;
...
public abstract class Configuration {
    ...
    private static final Log LOG = new ConsoleLog(Log.Levels.INFO, Configuration.class.getName());
    static {
        if (Classpaths.existFile("quick.properties")) {
            ...
            while (iterator.hasNext()) { // 覆盖默认配置
                ...
                LOG.info("Config {}: {}", kv.getKey(), kv.getValue());
            }
        }
    }
    ...
    public static void applySystemArgs(String... args) {
        for (String setting : args) {
            ...
            if (kv.length != 2) {
                System.err.println("System args have a unidentified config, discarded!");
                continue;
            }
