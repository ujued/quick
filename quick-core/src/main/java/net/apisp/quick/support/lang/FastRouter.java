...
public class FastRouter {
	...
	public Object route(HttpRequest request, HttpResponse response, WebContext webContext, Object executor) {
		...
		if (Runnable.class.isAssignableFrom(type)) {
			...
			return ((Function<WebContext, Object>) executor).apply(webContext);
		} else if (BiArgsRunnable.class.isAssignableFrom(type)){
			((BiArgsRunnable<HttpRequest, HttpResponse>)executor).run(request, response);
		} else {
			((ArgRunnable<WebContext>) executor).run(webContext);
		}
		return null;
	}
}
